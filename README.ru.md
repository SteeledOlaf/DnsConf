...
- Вы можете установить уникальное значение под каждый профиль, осуществив перечисление через запятую **без пробела**
...
+ Включите `dry_run`, чтобы загрузить и проверить источники, построить reconciliation plan и не менять DNS.

### Надёжность и безопасность

- Cloudflare использует смену поколений: новые списки и выключенные правила создаются до удаления старых. При ошибке
  новое поколение удаляется, а старое остаётся активным. На аккаунте должен быть запас по лимиту списков и правил для
  кратковременного существования двух поколений.
- Записи перенаправления из раздела `# Google AI` обновляются первыми с переиспользованием уже занятых программой
...
- Источники списков и DoH-донор должны использовать публичный HTTPS endpoint. Loopback, private, link-local и
  multicast-адреса отклоняются.
- Для Cloudflare используйте отдельный API Token только с минимальными разрешениями Zero Trust/Gateway Edit и
  ограничьте его нужным аккаунтом.
- Любая ошибка авторизации или API теперь завершает workflow с ошибкой; зелёный статус означает, что все запрошенные
  операции были подтверждены провайдером.
